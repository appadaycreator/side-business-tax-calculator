...
FAQ = [
    ("副業税金計算ツールは無料で使えますか？", "はい、完全無料・登録不要でご利用いただけます。"),
    ("副業収入が20万円以下なら確定申告は不要ですか？", "給与所得者の場合、副業の所得（収入−経費）が20万円以下なら所得税の確定申告は不要です。ただし住民税の申告が必要な場合があります。"),
    ("副業の経費として認められるものは？", "副業に直接関係する費用（PC・通信費・書籍・交通費など）が経費として認められます。家事按分（家賃・光熱費の一部）も副業割合分は経費にできます。"),
    ("フリーランスと副業では税務が違いますか？", "はい。フリーランス（事業所得）は青色申告特別控除（最大65万円）が使えますが、副業（雑所得）は使えません。年収300万円超の継続的な副業は事業所得になる場合があります。"),
    ("計算結果の精度はどの程度ですか？", "概算計算です。実際の税額は基礎控除・社会保険料控除・生命保険料控除などにより変わります。正確な税額は税理士または税務署にご確認ください。"),
]

HOW_TO = [
    "本業の年収（給与）を入力する",
    "副業の年間収入（売上総額）を入力する",
    "副業にかかった年間経費の見込みを入力する（ゼロなら0）",
    "副業の種類（雑所得・事業所得・不動産所得）を選択する",
    "「計算する」で追加税負担と確定申告の要否を確認する",
]
